...
class Checkin < ActiveRecord::Base
    ...
    def self.ok_to_checkin?(user_id, location_id, point_value)
        ...
        time_elapsed = self.time_since_last_checkin(user_id, location_id, point_value)
    ...
    def self.time_since_last_checkin(user_id, location_id, point_value)
        if Checkin.where(user_id: user_id, location_id: location_id).length > 0
            TimeDifference.between(Checkin.where(user_id: user_id, location_id: location_id).last.created_at, Time.now.getutc).in_minutes
        else
            0
        end
    end
